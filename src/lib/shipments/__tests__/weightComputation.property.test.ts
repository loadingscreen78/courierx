...
import { computeWeightKg } from '../bookingAdapter';
...
// Feature: booking-lifecycle-integration, Property 4: Weight computation matches type-specific formula

/**
 * Property 4: Weight computation matches type-specific formula
 *
 * For any valid form data and shipment type, computeWeightKg SHALL return:
 * - Medicine: sum(medicine.unitCount * 0.05) for all medicines
 * - Document: max(weight/1000, length*width*height/5000)
 * - Gift: sum(item.units * 0.5) for all items
 *
 * And the result SHALL always be positive.
 *
 * **Validates: Requirements 1.7**
 */

// ---------------------------------------------------------------------------
// Shared address generators (reused from bookingAdapter.property.test.ts)
// ---------------------------------------------------------------------------
...
describe('Property 4: Weight computation matches type-specific formula', () => {

  it('medicine weight equals sum(unitCount * 0.05) and is positive', () => {
    fc.assert(
      fc.property(medicineBookingDataArb, (formData) => {
        const result = computeWeightKg(formData, 'medicine');
        const expected = formData.medicines.reduce((sum, med) => sum + med.unitCount * 0.05, 0);
  ...
  it('document weight equals max(weight/1000, L*W*H/5000) and is positive', () => {
    fc.assert(
      fc.property(documentBookingDataArb, (formData) => {
        const result = computeWeightKg(formData, 'document');
        const actualKg = formData.weight / 1000;
        const volumetricKg = (formData.length * formData.width * formData.height) / 5000;
        const expected = Math.max(actualKg, volumetricKg);
  ...
  it('gift weight equals sum(units * 0.5) and is positive', () => {
    fc.assert(
      fc.property(giftBookingDataArb, (formData) => {
        const result = computeWeightKg(formData, 'gift');
        const expected = formData.items.reduce((sum, item) => sum + item.units * 0.5, 0);
