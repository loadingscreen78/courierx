...
import { MapPin, User, Phone, Home, Globe, Loader2, CheckCircle2 } from 'lucide-react';
...
const AddressStepComponent = ({ data, onUpdate }: AddressStepProps) => {
  ...
  const handleConsigneePhoneBlur = () => {
    ...
    if (countryInfo && !phone.startsWith('+') && phone.length > 3) {
      setLocalConsigneeAddress(prev => ({
        ...prev,
        phone: `${countryInfo.phone.dialCode} ${phone}`,
      }));
    }
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-2">
              ...
              {localPickupAddress.pincode && !validatePincode(localPickupAddress.pincode) && !pincodeError && (
